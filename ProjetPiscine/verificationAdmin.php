...
if ($db_found){
   ...
   $sql = "SELECT Nom, Prénom, Mdp FROM administrateur WHERE Email = '$username'"; 
   $result = mysqli_query($db_handle, $sql); // recherche de la requête dans la bdd 
   while($data = mysqli_fetch_assoc($result)){
      $nom = $data["Nom"]; 
      $prenom = $data["Prénom"];
      if ($data["Mdp"]== $mdp && !empty($data["Mdp"])){
   ...
   if($_SESSION["connexion"]== 0){
      $_SESSION["erreurConnexionAdmin"] = "Identifiant ou mot de passe incorrect"; 
